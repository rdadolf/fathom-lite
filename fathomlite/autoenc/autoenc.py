...
class AutoencBase(NeuralNetworkModel):
  ...
  def load_data(self):
    dl = DataLoader('/data')
    images = dl.load('mnist-inputs')
    # Unsupervised models don't use the labels.

    # Fathom-life uses the same training and test sets, since it has
    # negligible impact on performance.
    self.X_train, self.X_test = standard_scale(images, images)
  ...
  def run(self, runstep=default_runstep, n_steps=1):
    ...
    with self.G.as_default():
      ...
      for epoch in xrange(self.epochs):
        ...
        total_batch = DataLoader.number_of_samples // self.batch_size
        ...
        if not self.forward_only:
          avg_cost += loss_value * DataLoader.number_of_samples * self.batch_size
  ...
  def calc_total_cost(self, X):
    if self.forward_only:
      return self.session.run(self.loss, feed_dict = {self.xs: X})
    else:
